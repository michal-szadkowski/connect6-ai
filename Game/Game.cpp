...
Game::Game(Player& black, Player& white, std::shared_ptr<GameLogger> logger)
    : black(black), white(white), logger(logger)
{
    board = Board();
    black.SetColor(Color::Black);
    white.SetColor(Color::White);
    history = std::vector<std::pair<Board, Move>>();
}

Game::Game(Player& black, Player& white, const Board& board, std::shared_ptr<GameLogger> logger)
    : Game(black, white, logger) { this->board = board; }
...
Color Game::Play()
{
    if (board.GetResult() != Color::None)
        throw std::logic_error("can't start game with result");
...
bool Game::IsLegal(const Move& move)
{
    ...
    if (!board.IsEmpty(move.GetFirst()))
        return false;
    if (!board.ExpectingFullMove() && !move.IsHalf()) { return false; }
