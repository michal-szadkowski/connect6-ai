...
Move DqnPlayer::GetMove(const Board& board, const Move& prevMove)
{
    ...
    if (rememberActions)
        AddExperienceFromMove(board, {p1, p2, this->GetColor()});
    return {p1, p2, this->GetColor()};
}

StonePos DqnPlayer::GetPositionFromBoard(const Board& board)
{
    auto [result2, c] = agent.GetAction(board, eps);
...
void DqnPlayer::AddExperienceFromMove(const Board& board, const Move& move)
{
    Board b2 = board;
