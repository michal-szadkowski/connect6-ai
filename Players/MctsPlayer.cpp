...
MctsPlayer::MctsPlayer(std::shared_ptr<InfoLogger> logger, int explorations, int simulations, double expRate)
        : Player(std::move(logger)), expCount(explorations), simCount(simulations), expRate(expRate) {
...
Move MctsPlayer::GetMove(const Board &board, const Move &prevMove) {
    ...
    logger->WriteInfo(std::format("{:.5f} / {:d}", -tree->GetRoot()->GetScore(), tree->GetRoot()->GetVisitCount()));

    if (!s.second.get()) {
        move = {s.first->GetPosition(), StonePos::Empty(), this->GetColor()};
        logger->WriteInfo(std::format("{:.5f} / {:d}", s.first->GetScore(), s.first->GetVisitCount()));
    } else {
        move = {s.first->GetPosition(), s.second->GetPosition(), this->GetColor()};
        logger->WriteInfo(std::format("{:.5f} / {:d} {:.5f} / {:d} ",
