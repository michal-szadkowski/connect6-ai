...
struct FusedMBConvImpl : torch::nn::Module
{
    explicit FusedMBConvImpl(int ch, int expansion, int kernel)
    {
        ...
        inner = torch::nn::Sequential{
            torch::nn::Conv2d(torch::nn::Conv2dOptions(ch, expanded, kernel).padding(kernel / 2)), torch::nn::BatchNorm2d(expanded),
            torch::nn::ReLU(), torch::nn::Conv2d(torch::nn::Conv2dOptions(expanded, ch, 1)), torch::nn::BatchNorm2d(expanded),
        };
